        ...
        <div class='row form-group required'>
            ...
            <div class='col-md-6'>
                @foreach($regions as $region)
                    <label><input type="checkbox" name="regions[]" id="" value="{{ $region->id }}" @if(isset($camp->regions) && $camp->regions->contains($region->id)) checked @endif @if($region->applicants->count() || $region->roles->count()) disabled @endif title="學員{{ $region->applicants->count() }}，職務{{ $region->roles->count() }}"> {{ $region->name }}</label>
                @endforeach
            </div>
        </div>
        <div class='row form-group required'>
