        ...
        <div class='row form-group required'>
            ...
            <div class='col-md-6'>
                @foreach($regions as $region)
                    <label><input type="checkbox" name="regions[]" id="" value="{{ $region->id }}" @if(isset($camp->regions) && $camp->regions->contains($region->id)) checked @endif @if($camp->regions->where('region_id', $region->id)->get() && ($region->applicants->count() || $region->roles->count())) disabled @endif title="學員{{ $camp->applicants->where('region_id', $region->id)->count() }}，職務{{ $camp->roles->where('region_id', $region->id)->count() }}"> {{ $region->name }}</label>
                @endforeach
            </div>
        </div>
        <div class='row form-group required'>
