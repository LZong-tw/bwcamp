...
<br>
　　感謝您報名{{ $camp_data->fullName }}，報名手續已完成。請記下您的報名序號： <strong>{{ $applicant->id }}</strong>作為日後查詢使用。<br>
<br>
　　如您需修改報名表內容，可憑報名序號至 <a href="https://bwcamp.bwfoce.org/camp/{{ $applicant->batch->id }}/query"> 頁面修改</a>

　　我們將於 {{ \Carbon\Carbon::parse($campData->admission_announcing_date)->translatedFormat("Y年m月d日(l)") }} ($campData->admission_announcing_date_Weekday) 寄發錄取與否通知信件至您的電子信箱，或您可至活動網站 {{ $camp_data->site_url }} 查詢。<br>
　　<br>
