<div class="mt-2">
    ...
    <table class="table table-bordered table-hover"
        ...
        @forelse ($registeredVolunteers as $user)
            @if(!$currentUser->canAccessResource($user->application_log->first(), 'read', $camp, 'volunteerList'))
                @continue
            @endif
        ...
        @forelse ($applicants as &$applicant)
            @if(!$currentUser->canAccessResource($applicant, 'read', $camp, str_contains(url()->current(), 'volunteer') ? 'volunteer' : null))
                @continue
            @endif
            <tr @if($applicant->deleted_at) style="color: rgba(120, 120, 120, 0.4)!important" @endif>
                @if(($isSetting ?? false) || ($isSettingCarer ?? false))
