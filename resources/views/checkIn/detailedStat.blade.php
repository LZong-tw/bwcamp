...
<div class="container">
    ...
    <h3>全梯次</h3>
    <h4>累積報到人數 / 未報到人數：{{ $checkedInCount }} / {{ $applicantsCount - $checkedInCount }}&nbsp;&nbsp;&nbsp;&nbsp;報到率：{{ round($checkedInCount / $applicantsCount * 100, 2) }}%</h4>
    <hr>
    <div id="data">
        @foreach ($batchArray as $batch)
            ...
            <h4>累積報到人數 / 未報到人數：{{ $batch['checkedInApplicants'] }} / {{ $batch['allApplicants'] - $batch['checkedInApplicants'] }}&nbsp;&nbsp;&nbsp;&nbsp;報到率：{{ round($batch['checkedInApplicants'] / $batch['allApplicants'] * 100, 2) }}%</h4>
            @if(!$loop->last)
                <hr>
            @endif
        @endforeach
