<template>
    <form method="post">
        ...
        <input
            type="reset"
            value="清除篩選條件 - 顯示所有關懷員"
            class="btn btn-danger mb-3"
            onclick="window.location=window.location.href"
            v-if="isShowLearners && isShowVolunteers"
        />
        <input
            type="reset"
            value="清除篩選條件 - 顯示所有義工"
    ...
    methods: {
        ...
        filterSearch(column) {
            ...
            if (column != "roles") {
                ...
                this.theData = this.theData
                    .filter((item) => {
                        if (item["user"] && item["user"]["roles"]) {
                            return item["user"]["roles"].some((role) =>
                                role["section"]
                                    .toLowerCase()
                                    .includes(search.toLowerCase()),
                            );
                    ...
                    .map((element) => {
                        ...
                        if (newElt.user) {
                            return newElt.user["roles"].filter((subElement) =>
                                subElement["section"]
                                    .toLowerCase()
                                    .includes(search.toLowerCase()),
                            );
                ...
                this.theData = this.theData.filter((obj, index) => {
                    return (
                        index ===
                        this.theData.findIndex((o) => obj.section === o.section)
                    );
                });
                searchRole = 1;
            }
