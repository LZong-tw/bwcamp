...
test.describe('BWCamp Management System', () => {

  test('homepage loads correctly', async ({ page }) => {
    await page.goto('/');

    // Check if the page title contains expected text
    await expect(page).toHaveTitle(/綜合資料庫/);
 ...
 test('admin dashboard access', async ({ page }) => {
    ...
    // 檢查是否真的是管理頁面 (而不是錯誤頁面)
    await expect(page).not.toHaveTitle(/404|Not Found|Error/);

    // 檢查管理員元素 (強制要求存在)
    const adminContent = page.locator('.admin-dashboard, .management-panel, [data-test="admin-panel"]');
    await expect(adminContent.first()).toBeVisible(); // 移除 if 條件
  });
