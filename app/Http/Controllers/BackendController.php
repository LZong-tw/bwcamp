...
class BackendController extends Controller
{
    ...
    public function showLearners(Request $request)
    {
        ...
        $applicants = Cache::remember("camp{$this->campFullData->id}_applicants_viewed_backend_by_" . auth()->user()->id . "_query" . md5($queryStr) . "_issetting" . $isSetting, Config::get('cache.ttl'), function () use ($applicants_query) {
            $applicants = $applicants_query->get();
            return $applicants->each(fn ($applicant) => $applicant->id = $applicant->applicant_id);
        });
        if (!Cache::get("camp{$this->campFullData->id}_applicants_viewed_by_" . auth()->user()->id . "_query" . md5($queryStr) . "_issetting" . $isSetting)) {
            $applicants = $applicants->filter(fn ($applicant) => $this->user->canAccessResource($applicant, 'read', $this->campFullData, target: $applicant));
        }
    ...
    public function showVolunteers(Request $request)
    {
        ...
        $queryStr = !isset($queryStr) || $queryStr == null ? "" : $queryStr;
        $isSetting = $isSetting ?? false;
        $applicants = Cache::remember("camp{$this->campFullData->id}_volumeteeringApplicants_viewed_backend_by_" . auth()->user()->id . "_query" . md5($queryStr) . "_issetting" . $isSetting, Config::get('cache.ttl'), function () use ($applicants_query) {
            $applicants = $applicants_query->get();
            return $applicants->each(fn ($applicant) => $applicant->id = $applicant->applicant_id);
        });
        $registeredUsers = Cache::remember("camp{$this->campFullData->id}_registeredVolunteers_backend_viewed_by_" . auth()->user()->id . "_query" . md5($queryStr) . "_issetting" . $isSetting, Config::get('cache.ttl'), function () use ($registeredUsers) {
            return $registeredUsers->get();
        });
        if (!Cache::get("camp{$this->campFullData->id}_registeredVolunteers_viewed_by_" . auth()->user()->id . "_query" . md5($queryStr) . "_issetting" . $isSetting)) {
        ...
        if (!Cache::get("camp{$this->campFullData->id}_volumeteeringApplicants_viewed_by_" . auth()->user()->id . "_query" . md5($queryStr) . "_issetting" . $isSetting)) {
