...
class CampController extends Controller
{
    ...
    public function campIndex()
    {
        ...
        if ($today->lt($registration_start)) {
    ...
    public function campRegistration(Request $request)
    {
        ...
        [$fare_depart_from, $fare_back_to] = $this->trafficService->getTrafficFare($this->camp_data->table);
    ...
    public function campRegistrationFormSubmitted(Request $request)
    {
        ...
        if (isset($request->applicant_id) && !isset($request->useOldData2Register)) {
            ...
            if (isset($formData['depart_from']) || isset($formData['back_to'])) {
                $this->trafficService->updateApplicantTraffic($applicant, $this->camp_data->table, $formData['depart_from'] ?? null, $formData['back_to'] ?? null);
            }
            if (isset($formData['room_type'])) {
                $this->lodgingService->updateApplicantLodging($applicant, $this->camp_data, $formData['room_type'], $formData['nights'] ?? null);
            }
        ...
        // 營隊報名
            ...
            if ($request->required_name || $request->required_filename) {
                ...
                \Sentry::captureMessage('Registration from camp ID: ' . $this->camp_data->id);
                $request = $this->campDataService->checkBoxToArray($request);
                $formData = $request->toArray();
    ...
    public function campQueryAdmission(Request $request)
    {
        ...
        if ($request->name != null && $request->sn != null) {
            ...
            //只用序號去找，後面會看名字是否相符
            $applicant = Applicant::with([$campTable, 'lodging', 'traffic'])->withTrashed()->findOrFail($request->sn);
        }
        //檢查條件：applicant存在；查詢到的名字與序號相符；找到的人是報名本營隊
        if ($applicant && $applicant->name == $request->name &&
            $applicant->camp->id == $this->camp_data->id )
        {
            //使用報名者的報名日期來計算費率，避免修改資料後費率變動的問題
            $fare_room = $this->lodgingService->getLodgingFare($this->camp_data, $applicant->created_at);
            [$fare_depart_from, $fare_back_to] = $this->trafficService->getTrafficFare($campTable);
