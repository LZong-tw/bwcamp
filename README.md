...
### 初始化
1. 建立營隊
2. 建立梯次、**指定梯次學員組別數量（將會連帶影響關懷組數量）**
3. 建立組織及相關職務（義工組別、職務組別）
4. 設定各職務的權限
### 義工
1. 報名
2. 報名確認信
3. 後台錄取 + **檢查是否有註冊，並顯示選項（連結既有帳號或產生新帳號）** + 分組（各個職務組別）
    + _**顯示「報名義工」 + 「登入帳號」，名單需針對登入帳號及報名義工做整理**_
    + 關懷員的部分，在這裡只會把關懷員分組，不會指定他們內部的職位
4. 現場報到 / 課程簽到
   #### 關懷員
   0. 要先被指派為關懷小組組員
   1. 會在關懷員名單做進一步指派職位
   2. 關懷小組職位：小組長、副小組長、關懷員、組員（待分配職位的人）
### 學員
1. 報名（一般管道）、後台報名（特殊管道、邀請制）
2. 報名確認信
3. 後台錄取 + 分組（學員組別）
    + **取消報名的放後面並用顏色區分**
4. 寄發錄取通知信
5. 回覆參加意願
   + 企業營菁英營：在後台幫學員處理 → 需增加狀態：**已聯絡，未確定是否參加**
6. 現場報到 / 課程簽到
## 各界面需加入權限判斷功能
## 綜合操作界面顯示問題待修正
